# -*- coding: utf-8 -*-
"""Object Localization with TensorFlow.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/12Y4xqMaZzKZE4EW1PBm9tqEUApPIR5yR
...
def create_example():
    ...
    img = np.ones((144,144,3))*255
    row = np.random.randint(0,72)
    col = np.random.randint(0,72)
    img[row:row+72,col:col+72,:]=np.array(emojis[class_id]['image'])
    img = img.astype('uint8')
    return img, class_id, (row+10)/144, (col+10)/144        #around 10 pixcels of emojis are not utilised from borders.
...
model.compile(
    loss = {'class_out':'categorical_crossentropy', 'box_out': 'mse'}, optimizer = tf.keras.optimizers.Adam(1e-3),
...
def test_model(model, test_datagen):
    ...
    # We will use a batch size of 1 for testing purpose
    ...
    image = plot_bounding_box(image, gt_coords, pred_coords=pred_coords, norm = True)
    color = 'green' if gt == pred_class_name else 'red'
    plt.imshow(image)
    plt.xlabel(f'Ground Truth: {gt}', color = color)
    plt.ylabel(f'Prediction: {pred_class_name}', color=color)
    plt.xticks([])
    plt.yticks([])
...
model.fit(data_generator(), epochs = 20, steps_per_epoch=500, callbacks=[
                                                                         ShowTestImages(),
                                                                         tf.keras.callbacks.EarlyStopping(monitor='box_out_iou', mode = 'max', patience = 3),
